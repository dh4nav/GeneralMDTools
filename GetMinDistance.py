...
parser.add_argument("-d", "--distance", help="Distanc threshold", required=True)
...
for n, ec in enumerate(xyz_iter):
    listlen = len([x > 0.1 and x < args.distance for x in xyzt.GetDistsDiff(ec, ec)]) 
    if listlen:
        opf.write(str(n) + " " + str(listlen) + "\n")
